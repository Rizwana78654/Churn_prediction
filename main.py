...
class CHURN:
    def __init__(self, path):
        try:
            ...
            self.df['SIM_provider'] = np.random.choice(['Jio', 'Airtel', 'VI', 'BSNL'], size=len(self.df),
                                                       p=[0.4, 0.3, 0.2, 0.1])
            logger.info(f'total data size {self.df.shape}')
    ...
    def cat_to_num(self):
        try:
            ...
            logger.info(
                f'Training data columns : {self.training_data.columns} and its shape : {self.training_data.shape}')
            logger.info(f'testing data columns : {self.testing_data.columns} and its shape : {self.testing_data.shape}')
        except Exception as e:
            err_type, err_msg, err_line = sys.exc_info()
            logger.info(
                f'Error in lineno {err_line.tb_lineno} and error is due to {err_type} and the error message is {err_msg}')
